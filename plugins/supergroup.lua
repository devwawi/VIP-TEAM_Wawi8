...
local function run(msg, matches)
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'Vbot' and not matches[2] then
		...
		if matches[1] == 'Tbot' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == "adinslist" then
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested group adminslist")
			return adminslist(msg)
		...
		if matches[1] == "mall" and not matches[2] and is_momod(msg) then
		...
		if matches[1] == 'dmsg' and is_momod(msg) then
		...
		if matches[1] == 'ban' or matches[1] == 'dee' and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'ban' or matches[1] == 'dee' and string.match(matches[2], '^%d+$') then
	...
	if matches[1] == 'kickme' then
		...
		if matches[1] == 'nlink' and is_momod(msg)then
		...
		if matches[1] == 'addpro' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'addpro',
					msg = msg
			...
			elseif matches[1] == 'addpro' and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local	get_cmd = 'addpro'
				local	msg = msg
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'addpro' and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'addpro'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'unpro' then
			...
			elseif matches[1] == 'unpro' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'unpro'
		...
		if matches[1] == "susername" and is_admin1(msg) then
		...
		if matches[1] == 'de' then
			...
			if matches[2] == 'admins' then
				...
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] cleaned adminslist")
				return 'adinslist has been cleaned✅ \n👮 Order by > @'..msg.from.username
			end
			...
			if matches[2] == 'shhlist' then
				chat_id = msg.to.id
				local hash =  'mute_user:'..chat_id
					redis:del(hash)
				return "shhlist Cleaned✅ \n👮 Order by > @"..msg.from.username
			end
		...
		if matches[1] == 'v' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1] == 't' and is_momod(msg) then
			local target = msg.to.id
			     if matches[2] == 'all' then
      	local dsafemode ={
        unlock_group_links(msg, data, target),
		...
		if matches[1] == 'v' and is_owner(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == "medialist" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == "shhlist" and is_momod(msg) then
			local chat_id = msg.to.id
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested SuperGroup mutelist")
			return muted_user_list(chat_id)
		end

		if matches[1] == 'setg' and is_momod(msg) then
			local target = msg.to.id
